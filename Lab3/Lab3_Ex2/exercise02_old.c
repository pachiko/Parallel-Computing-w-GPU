...
int main(int argc, char *argv[])
{
	...
	{
		// Section 2.2.2
		// Initialize local histogram. Only one thread should do it, doesn't matter who.
		// Make sure this is done before going forward (barrier).
		const int threads = omp_get_max_threads();
		#pragma omp master
		{
			size_t sz = sizeof(int*) * threads;
			local_histograms = malloc(sz);
			//printf("%d, %d\n", threads, sz);
			size_t r = sizeof(histogram);
			//printf("%d\n", r);
			for (int k = 0; k < threads; k++) {
				local_histograms[k] = malloc(r);
				memset(local_histograms[k], 0, r);
				//printf("Malloc-ed for thread %d\n", k);
			}
		}
		#pragma omp barrier

		#pragma omp for private(x, i, n_r, n_i, o_r, o_i, c_r, c_i, mu)
		for (y = 0; y < HEIGHT; y++)
			for (x = 0; x < WIDTH; x++)
			{
		...
		// Section 2.2.2
		// Sum the local histograms into global histogram
		#pragma omp barrier
		#pragma omp master
		{
			//printf("Accumulate...\n");
