...
__global__ void keyValues(particles *p, key_values *kv)
{
	...
	float2 loc = p->location[idx];
	int bin = binIndex(binLocation(loc));
	kv->sorting_key[idx] = bin;
	kv->value[idx] = idx;
	
}

__global__ void reorderParticles(key_values* kv, particles* p, particles* sorted) {
	int i = blockIdx.x * blockDim.x + threadIdx.x;

	int idx = kv->value[i];
	float2 loc = p->location[idx];
	sorted->location[i] = loc;
}
...
__global__ void histogramParticles(particles* p, environment* env) {
	...
	if (useGlobal) {
		int bin = binIndex(binLocation(p->location[i]));
		atomicAdd(env->count + bin, 1);
	}
	else {
		__shared__ int shared_count[ENV_BINS]; // shared count of particles in bins

		if (threadIdx.x == 0) {
			for (int j = 0; j < ENV_BINS; j++) { // initialize counts
				shared_count[j] = 0;
			}
		}
		__syncthreads();
		...
		if (bin < ENV_BINS && bin >= 0) {
			// The bin < ENV_BINS check is very important. There are particles outside env.
			// Causes illegal memory issues in Thrust later.
			// Why only for shared memory? Doesn't trigger anything for global memory.
			// See keyValues kernel and useGlobal block.
			// But there is bounds-checking in particleNNSearch kernel.

			atomicAdd(&shared_count[bin], 1); // local accumulation
		}
		__syncthreads();

		if (threadIdx.x == 0) {
			for (int j = 0; j < ENV_BINS; j++) { // accumulate counts to global
				atomicAdd(env->count + j, shared_count[j]);
			}
		}
	}
}
...
void particlesGPU()
{
	...
	thrust::device_ptr<int> keys_first = thrust::device_pointer_cast(d_key_values->sorting_key);
	thrust::device_ptr<int> keys_last = thrust::device_pointer_cast(d_key_values->sorting_key + NUM_PARTICLES);
	thrust::device_ptr<int> values_first = thrust::device_pointer_cast(d_key_values->value);
	...
	keys_first = thrust::device_pointer_cast(d_env->count);
	keys_last = thrust::device_pointer_cast(d_env->count + ENV_BINS);
	values_first = thrust::device_pointer_cast(d_env->start_index);
	thrust::exclusive_scan(keys_first, keys_last, values_first, 0);
