#ifndef __CUDACC__
#define __CUDACC__ // see cuda_texture_types.h
#endif
...
texture<uchar4, cudaTextureType1D, cudaReadModeElementType> texture1D;
texture<uchar4, cudaTextureType2D, cudaReadModeElementType> texture2D;
	...
	for (int i = -SAMPLE_SIZE; i <= SAMPLE_SIZE; i++) {
		for (int j = -SAMPLE_SIZE; j <= SAMPLE_SIZE; j++) {
			...
			if (y_offset >= IMAGE_DIM)
				y_offset -= IMAGE_DIM;
			int offset = x_offset + y_offset * blockDim.x * gridDim.x;
			pixel = tex1Dfetch(texture1D, offset);
...
__global__ void image_blur_texture2D(uchar4* image_output) {
	...
	for (int i = -SAMPLE_SIZE; i <= SAMPLE_SIZE; i++) {
		for (int j = -SAMPLE_SIZE; j <= SAMPLE_SIZE; j++) {
			...
			//if (y_offset >= IMAGE_DIM)
			//	y_offset -= IMAGE_DIM;
			pixel = tex2D(texture2D, x_offset, y_offset);
...
int main(void) {
	...
	// Texture 1D version
	cudaEventRecord(start, 0);
	cudaBindTexture(0, texture1D, d_image, image_size);
	image_blur_texture1D << <blocksPerGrid, threadsPerBlock >> > (d_image_output);
	cudaUnbindTexture(texture1D);
	cudaEventRecord(stop, 0);
	cudaEventSynchronize(stop);
	cudaEventElapsedTime(&ms.y, start, stop);
	checkCUDAError("kernel texture 1D");

	// Texture 2D version
	cudaEventRecord(start, 0);
	cudaChannelFormatDesc desc = cudaCreateChannelDesc<uchar4>();
	cudaBindTexture2D(0, texture2D, d_image, desc, IMAGE_DIM, IMAGE_DIM, IMAGE_DIM*sizeof(uchar4));
	texture2D.addressMode[0] = cudaAddressModeClamp; // also works for cudaTextureObject_t
	...
	checkCUDAError("kernel texture 2D");
